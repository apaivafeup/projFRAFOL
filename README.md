...
Clone the project into a directory of your choice using the following command: `git clone --branch FRAFOL2.0 https://github.com/apaivafeup/projFRAFOL.git`
...
![FRAFOL Landing Page](./assets/2.0/landing.png "Landing Page")
...
![Project Working Page](./assets/2.0/analyzer.png "Project Page")

This page includes an editor for writing test cases and a dashboard of information regarding mutation testing.

When the user first enters this board on a new opened project, the first mutation analysis will be performed, thus the Mutate button will be Loading.

Consequently, please wait while we quickly setup the base mutants of the project, so that the user can tackle them efficiently.

While waiting, one can already start analyzing and writing tests.

For the navigation tabs inside this page, check this [reference information](#navigation-tabs)

### Kill Matrix

![Kill Matrix](./assets/2.0/killMatrix.png "Kill Matrix Page")
...
After writing some tests, the user can generate the kill matrix and obtain information about the mutants that were killed.

There is a relevant "?" section on the top right of the screen which is helpful before generating a kill matrix.

### Student

An user can use the tool without authenticating. However, if the user is a student, and the teacher has a class created in FRAFOL, there is a special tab dedicated to this.

As a student, create an account with your email, student number, and password, and select your class.

The student will have to be accepted by the teacher to be admitted into the system.

After being admitted, the student can submit their tests relative to a specific project/tool. These will be seen and evaluated by the class teacher.

The user can submit multiple times, however, the dates are always considered on the server, so the teacher can view when the submission was inserted.

If an user submits, for a specific project and tool, the last submission is overwritten.

### Teacher

If you would like to use our tool for evaluation purposes and have your own student class, please email apaiva@up.pt for registering.

Once the teacher is admitted, the user can create a new class, which will then be avaliable for their students to select once the latter ones register into the system.

The teacher can then admit or reject these students.

Furthermore, the teacher view has detailed information about:

- How many students and submissions there are in a specific class
- All submission details, including killed mutants and code, in a dashboard-like page
- Kill Matrixes for each of the students, as well as a leaderboard.
